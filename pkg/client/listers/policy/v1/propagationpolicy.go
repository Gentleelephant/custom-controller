...
type PropagationPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.PropagationPolicy, err error)
...
// List lists all PropagationPolicies in the indexer.
func (s *propagationPolicyLister) List(selector labels.Selector) (ret []*v1.PropagationPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.PropagationPolicy))
	})
	return ret, err
}
...
type PropagationPolicyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.PropagationPolicy, error)
...
// List lists all PropagationPolicies in the indexer for a given namespace.
func (s propagationPolicyNamespaceLister) List(selector labels.Selector) (ret []*v1.PropagationPolicy, err error) {
...
// Get retrieves the PropagationPolicy from the indexer for a given namespace and name.
func (s propagationPolicyNamespaceLister) Get(name string) (*v1.PropagationPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("propagationpolicy"), name)
	}
	return obj.(*v1.PropagationPolicy), nil
}
