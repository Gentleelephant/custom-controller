...
type OverridePolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.OverridePolicy, err error)
...
// List lists all OverridePolicies in the indexer.
func (s *overridePolicyLister) List(selector labels.Selector) (ret []*v1.OverridePolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.OverridePolicy))
	})
	return ret, err
}
...
type OverridePolicyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.OverridePolicy, error)
...
// List lists all OverridePolicies in the indexer for a given namespace.
func (s overridePolicyNamespaceLister) List(selector labels.Selector) (ret []*v1.OverridePolicy, err error) {
...
// Get retrieves the OverridePolicy from the indexer for a given namespace and name.
func (s overridePolicyNamespaceLister) Get(name string) (*v1.OverridePolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("overridepolicy"), name)
	}
	return obj.(*v1.OverridePolicy), nil
}
