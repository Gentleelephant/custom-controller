...
type ClusterOverridePolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ClusterOverridePolicy, err error)
	// Get retrieves the ClusterOverridePolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ClusterOverridePolicy, error)
...
// List lists all ClusterOverridePolicies in the indexer.
func (s *clusterOverridePolicyLister) List(selector labels.Selector) (ret []*v1.ClusterOverridePolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterOverridePolicy))
	})
	return ret, err
}

// Get retrieves the ClusterOverridePolicy from the index for a given name.
func (s *clusterOverridePolicyLister) Get(name string) (*v1.ClusterOverridePolicy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusteroverridepolicy"), name)
	}
	return obj.(*v1.ClusterOverridePolicy), nil
}
