...
type ClusterPropagationPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ClusterPropagationPolicy, err error)
	// Get retrieves the ClusterPropagationPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ClusterPropagationPolicy, error)
...
// List lists all ClusterPropagationPolicies in the indexer.
func (s *clusterPropagationPolicyLister) List(selector labels.Selector) (ret []*v1.ClusterPropagationPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterPropagationPolicy))
	})
	return ret, err
}

// Get retrieves the ClusterPropagationPolicy from the index for a given name.
func (s *clusterPropagationPolicyLister) Get(name string) (*v1.ClusterPropagationPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusterpropagationpolicy"), name)
	}
	return obj.(*v1.ClusterPropagationPolicy), nil
}
