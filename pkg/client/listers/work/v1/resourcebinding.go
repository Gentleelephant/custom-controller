...
type ResourceBindingLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ResourceBinding, err error)
...
// List lists all ResourceBindings in the indexer.
func (s *resourceBindingLister) List(selector labels.Selector) (ret []*v1.ResourceBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ResourceBinding))
	})
	return ret, err
}
...
type ResourceBindingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ResourceBinding, error)
...
// List lists all ResourceBindings in the indexer for a given namespace.
func (s resourceBindingNamespaceLister) List(selector labels.Selector) (ret []*v1.ResourceBinding, err error) {
...
// Get retrieves the ResourceBinding from the indexer for a given namespace and name.
func (s resourceBindingNamespaceLister) Get(name string) (*v1.ResourceBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("resourcebinding"), name)
	}
	return obj.(*v1.ResourceBinding), nil
}
