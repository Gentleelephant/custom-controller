...
type WorkLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Work, err error)
...
// List lists all Works in the indexer.
func (s *workLister) List(selector labels.Selector) (ret []*v1.Work, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Work))
	})
	return ret, err
}
...
// List lists all Works in the indexer for a given namespace.
func (s workNamespaceLister) List(selector labels.Selector) (ret []*v1.Work, err error) {
...
// Get retrieves the Work from the indexer for a given namespace and name.
func (s workNamespaceLister) Get(name string) (*v1.Work, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("work"), name)
	}
	return obj.(*v1.Work), nil
}
